import {
  ...
  import {
	...
	sendMessage,
	inviteForPlay,
  } from "./chatServices/liveChatService.js";
  import {
	showSuggestions,
	...
	unblockedElements,
	updateFriendsList,
  } from "./chatServices/uiService.js";
  ...
  function addEventListeners(elements, socketActivate, currentUser, otherUser) {
	...
	elements["search-friend"].addEventListener("focus", () =>
	  handleSearchFocus(elements)
	);
	elements["search-friend"].addEventListener("blur", () =>
	...
	elements["add-friend"].addEventListener("click", () =>
	  addFriend(elements["search-friend"].value.trim(), elements["friends-list"])
	);
	elements["search-friend"].addEventListener("keydown", (event) =>
	  handlePushEnterFriend(
		event,
		elements["search-friend"].value.trim(),
		elements["friends-list"]
	  )
	);

	// Handle friend requests
	elements["requests-list"].addEventListener("click", (event) =>
	  handleRequestsClick(event, elements)
	);

	// Handle friends list clicks
	elements["friends-list"].addEventListener("click", (event) =>
	  handleFriendsListClick(event, socketActivate, currentUser, otherUser)
	);

	// Handle send message
	elements["send-message"].addEventListener("click", () =>
	  handleSendMessage(socketActivate, currentUser, elements["message-input"])
	);
	elements["message-input"].addEventListener("keydown", (event) =>
	  handlePushEnterMessage(
		event,
		socketActivate,
		currentUser,
	...
	elements["block-friend"].addEventListener("click", () =>
	  handleBlockFriend(socketActivate, currentUser, otherUser)
	);

	// Handle play click
	elements["invite-friend"].addEventListener("click", () =>
	  handleInvitePlay(socketActivate, currentUser)
	);
  ...
  function handleSearchFocus(elements) {
	...
	if (currentValue) handleSearchInput(currentValue, elements);
	else elements["search-friend"].placeholder = "";
  }
  ...
  async function handleSearchInput(query, elements) {
	const results = await searchFriends(
	  query,
	  elements["suggestions-container"],
	  elements["search-friend"]
	);
	showSuggestions(
	  results,
	  elements["suggestions-container"],
  ...
  function hideDropDownList(event, elements) {
	if (
	  !elements["search-friend"].contains(event.target) &&
	  !elements["suggestions-container"].contains(event.target)
	)
  ...
  function handlePushEnterFriend(event, friendToAdd, friendsList) {
	if (event.key === "Enter" && friendToAdd) addFriend(friendToAdd, friendsList);
  }
  ...
  async function handleFriendsListClick(
	event,
	socketActivate,
	currentUser,
	otherUser
	...
	try {
	  ...
	  if (otherUser.id === friend.dataset.friendId) return;
	  ...
	  const response = await api.apiFetch(
		"chat/is_blocked/?id=" + otherUser.id,
		true,
		"GET"
	  );
  ...
  function handleSendMessage(socketActivate, currentUser, messageInput) {
	if (Object.keys(socketActivate).length === 0) {
	  const toast = new Toast(
		"Error",
		"Veuillez selectionner un ami avant d'envoyer un message",
		"error"
	  );
	  toast.show();
	  return;
  ...
  function handlePushEnterMessage(
	event,
	socketActivate,
	currentUser,
	messageInput
  ...
  async function handleAcceptPlay(event, currentUser, socketActivate) {
	try {
	  ...
	  await api.apiFetch("pong/match/individual/accept/", true, "POST", {
		match_id: matchId,
	  });
	  const payload = {
  ...
  function handleRefusePlay(event, currentUser, socketActivate) {
	try {
	  ...
	  api
		.apiFetch("pong/match/individual/refuse/", true, "POST", {
		  match_id: matchId,
		})
