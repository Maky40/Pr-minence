...
const waitForPlayerAsync = () => {
  return new Promise((resolve) => {
    const interval = setInterval(() => {
      if (pong42.player && pong42.player.id) {
        // Vérifie si player est défini
        clearInterval(interval); // Arrête l'intervalle
        resolve(); // Résout la promesse quand pong42.player est défini
      }
    }, 100); // Vérifie toutes les 100ms
    pong42.registerInterval(interval); // Enregistre l'intervalle pour le nettoyer plus tard
  });
};
...
async function setStats(profileStatsContainer) {
  ...
  const tournament_match = data.matches.filter(
    (match) => match.tournament_match
  );
...
function setStats2(match) {
  ...
  match.forEach((match) => {
    ...
    if (myStats.is_winner) victories++;
    else defeats++;
  });
