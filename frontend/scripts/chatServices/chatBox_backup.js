...
export function init() {
  ...
  if (tournamentButton && privateButton) {
    tournamentButton.addEventListener("click", () =>
      showChat("tournament", friendsList)
    );
    privateButton.addEventListener("click", () =>
      showChat("private", friendsList)
    );
  }
  ...
  blockFriendButton.addEventListener("click", () => {
    if (currentChatFriend) {
      ...
      blockFriendButton.textContent = currentChatFriend.blocked
        ? "Débloquer"
        : "Bloquer";
      blockFriendButton.classList.toggle(
        "btn-danger",
        !currentChatFriend.blocked
      );
      blockFriendButton.classList.toggle(
        "btn-secondary",
        currentChatFriend.blocked
  ...
  document.addEventListener("click", (event) => {
    if (
      !searchFriendInput.contains(event.target) &&
    ...
    if (event.target.classList.contains("friend-name")) {
      ...
      try {
        ...
        const socket = new WebSocket(
          `wss://${API_URL}/chat/ws/chat/${otherUserId}/`
        );
  ...
  data.friendships.forEach((friend) => {
    ...
    const friendItem = `
			  <li class="list-group-item d-flex justify-content-between align-items-center">
				  <span class="friend-name" data-username="${friend.id}">${
      friend.username
    }</span>
				  <span class="badge ${
            api === "requests"
...
async function addFriend(searchFriendInput, friendsList) {
  ...
  try {
    const response_id = await api.apiFetch(
...
async function requestFriend(id, friendsList) {
  ...
  const response_add = await api.apiFetch(
    "/player/friendship/",
    true,
    "POST",
    request_id
...
async function searchFriends(query, suggestionsContainer, searchFriendInput) {
  ...
  const results = await api.apiFetch(
    "/player/search-players/?username=" + query,
    true,
    "GET"
  );
  console.log(results);
